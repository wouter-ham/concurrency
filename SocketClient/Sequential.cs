...
namespace Sequential
{
    ...
    public class SimpleClient
    {
        ...
        public SimpleClient(int id, Setting settings)
        {
            ...
            cmd_message = "ClientId=" + client_id + settings.command_msg_sep + cmd;
        ...
        {
            var messageReceived = new byte[bufferSize];
            var numBytes = 0;
            string rcvdMsg = null;
            var reply = "";
            var stop = false;
            ...
            try
            {
                ...
                Console.WriteLine("[Client] connected to --> {0} ", clientSocket.RemoteEndPoint);
        ...
        public void sendMessage(string msg)
        {
            ...
            var messageSent = Encoding.UTF8.GetBytes(msg);
            var byteSent = clientSocket.Send(messageSent);
        }
    }
    ...
    public class SequentialClientsSimulator
    {
        ...
        public SequentialClientsSimulator()
        {
            ...
            for (var i = 0; i < settings.experimentNumberOfClients; i++)
        ...
        {
            Console.Out.WriteLine("\n[ClientSimulator] Sequential simulator is going to start with {0}",
                settings.experimentNumberOfClients);
            for (var i = 0; i < settings.experimentNumberOfClients; i++)
            {
            ...
            var
                endClient = new SimpleClient(-1,
                    settings); // this is a terminating client: it will terminate the whole simulation
